...
-   This is achieved using the Nadaraya-Watson method and cross-validation to select smoothing parameters with *categorical* kernel functions [@LI_OUYANG_RACINE:2009] or *continuous* kernel functions [@BIERENS:1983]
...
-   Otherwise, when kernel smoothing under common regularity conditions, we attain a $\sqrt{nh}$ rate of convergence (holds for local polynomial estimators with ad hoc fixed order $p$)
